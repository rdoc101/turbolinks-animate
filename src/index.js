window.TurbolinksAnimate =
  ...
  new (function() {
    ...
    this.animations = [
      ...
      {
        name: 'bounceInDown',
        disappear: 'bounceOutDown',
        reverse: 'bounceInUp'
      },
      { name: 'bounceOutDown', disappear: true, reverse: 'bounceOutUp' },
      {
        name: 'bounceInLeft',
        disappear: 'bounceOutLeft',
        reverse: 'bounceInRight'
      },
      { name: 'bounceOutLeft', disappear: true, reverse: 'bounceOutRight' },
      {
        name: 'bounceInRight',
        disappear: 'bounceOutRight',
        reverse: 'bounceInLeft'
      },
      ...
      { name: 'rotateOut', disappear: true, reverse: null },
      {
        ...
        disappear: 'rotateOutDownLeft',
        reverse: 'rotateInUpRight'
      },
      {
        name: 'rotateOutDownLeft',
        disappear: true,
        reverse: 'rotateOutUpRight'
      },
      {
        name: 'rotateInDownRight',
        disappear: 'rotateOutDownRight',
        reverse: 'rotateInUpLeft'
      },
      {
        name: 'rotateOutDownRight',
        disappear: true,
        reverse: 'rotateOutUpLeft'
      },
      {
        name: 'rotateInUpLeft',
        disappear: 'rotateOutUpLeft',
        reverse: 'rotateInDownRight'
      },
      {
        name: 'rotateOutUpLeft',
        disappear: true,
        reverse: 'rotateOutDownRight'
      },
      {
        name: 'rotateInUpRight',
        disappear: 'rotateOutUpRight',
        reverse: 'rotateInDownLeft'
      },
      {
        name: 'rotateOutUpRight',
        disappear: true,
        reverse: 'rotateOutDownLeft'
      },
      ...
      {
        name: 'slideInLeft',
        disappear: 'slideOutLeft',
        reverse: 'slideInRight'
      },
      { name: 'slideOutLeft', disappear: true, reverse: 'slideOutRight' },
      {
        name: 'slideInRight',
        disappear: 'slideOutRight',
        reverse: 'slideInLeft'
      },
    ...
    this.init = options => {
      ...
      if (
        window.TurbolinksAnimate.initialized == false &&
        options.customListeners == false
      ) {
        ...
        document
          ...
          .forEach(element =>
            element.addEventListener('click', () => (ignoreBeforeunload = true))
          );
        window.addEventListener('beforeunload', () => {
          if (!ignoreBeforeunload) window.TurbolinksAnimate.disappear();
      ...
      document.querySelectorAll('a, button').forEach(element => {
        element.addEventListener('click', () => {
          ...
          window.TurbolinksAnimate.options.animation =
            element.dataset.turbolinksAnimateAnimation || options.animation;
          window.TurbolinksAnimate.options.appear =
            element.dataset.turbolinksAnimateAppear;
          window.TurbolinksAnimate.options.duration =
            element.dataset.turbolinksAnimateDuration || options.duration;
          window.TurbolinksAnimate.options.delay =
            element.dataset.turbolinksAnimateDelay || options.delay;
          window.TurbolinksAnimate.options.type =
            element.dataset.turbolinksAnimateType;
        });
      });

      window.TurbolinksAnimate.initialized = true;
      if (options.customListeners == false) window.TurbolinksAnimate.appear();
    };
    ...
    this.prepareTransition = newBody => {
      document
        .querySelectorAll('[data-turbolinks-animate-transition]')
        .forEach(element => {
          let properties = element.dataset.turbolinksAnimateTransition.split(
              ','
            ),
            matchingElements = newBody.querySelectorAll(
              element.tagName + '[data-turbolinks-animate-transition]'
            ),
            newElement = null;
    ...
    this.getElements = () => {
      ...
      function getChildren(parent) {
        let type =
          window.TurbolinksAnimate.options.type ||
          window.TurbolinksAnimate.options.previousType ||
          'true';
        if (parent.dataset.turbolinksAnimatePersist == type) {
          return;
        } else if (
          parent.dataset.turbolinksAnimatePersistItself == type ||
          parent.querySelectorAll('[data-turbolinks-animate-persist]').length >
            0 ||
          parent.querySelectorAll('[data-turbolinks-animate-persist-itself]')
            .length > 0
        ) {
    ...
    this.resetClasses = () => {
      if (window.TurbolinksAnimate.elements != null) {
        window.TurbolinksAnimate.elements.forEach(element => {
          window.TurbolinksAnimate.animateClasses.forEach(animation =>
    ...
    this.animate = () => {
      ...
      window.TurbolinksAnimate.element.addEventListener(
        ...
        event => {
          ...
          dispatchEvent('turbolinks:animation-end', {
            detail: {
              element: window.TurbolinksAnimate.element,
              disappearing: window.TurbolinksAnimate.disappearing
            }
          });
        }
      );

      dispatchEvent('turbolinks:animation-start', {
        detail: {
          element: window.TurbolinksAnimate.element,
          disappearing: window.TurbolinksAnimate.disappearing,
          animation: animation
      ...
      window.TurbolinksAnimate.elements.forEach(element => {
        ...
        window.TurbolinksAnimate.getClassListFor(animation).forEach(animation =>
    ...
    this.getAnimation = () => {
      ...
      if (window.TurbolinksAnimate.inline) {
        ...
        typeof window.TurbolinksAnimate.element.dataset
          .turbolinksAnimateAnimation !== 'undefined'
      ) {
        animation =
          window.TurbolinksAnimate.element.dataset.turbolinksAnimateAnimation;
      } else {
        animation = window.TurbolinksAnimate.options.animation;
      }
    ...
    this.getClassListFor = animations => {
      ...
      let breakpoints = window.TurbolinksAnimate.options.breakpoints.sort(
        (a, b) => {
      ...
      if (animation == null) animation = animations;

      animation = window.TurbolinksAnimate.animations.filter(
        object => object.name.toLowerCase() == animation.toLowerCase()
      )[0];
      if (window.TurbolinksAnimate.disappearing) {
        if (animation.disappear != true)
          animation = window.TurbolinksAnimate.animations.filter(
            object =>
              object.name.toLowerCase() == animation.disappear.toLowerCase()
          )[0];
        if (
          window.TurbolinksAnimate.options.reversedDisappearing &&
          animation.reverse != null
        ) {
